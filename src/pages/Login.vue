<template>
	<auth-box title="Log in">
		<p>Username</p>
		<input type="text" placeholder="Username" v-model="username" />
		<p v-if="usernameErrorMessage" class="alert">
			{{ usernameErrorMessage }}
		</p>
		<p>Password</p>
		<input type="password" placeholder="Password" v-model="password" />
		<p v-if="passwordErrorMessage" class="alert">
		...
		Login() {
			...
			axios
				...
				.then(res => {
					...
					if (UserV == 0) this.usernameErrorMessage = "User not registered";
					else if (PassV == 0) this.passwordErrorMessage = "Incorrect Password";
				})
